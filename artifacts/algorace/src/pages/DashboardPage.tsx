      ...
      <main className="max-w-7xl mx-auto px-4 py-8 space-y-8">
        <section className="flex flex-col md:flex-row gap-6 items-start justify-between">
          <div className="space-y-2">
            <h1 className="text-3xl font-bold tracking-tight" data-testid="text-welcome">Welcome back, {user?.name}!</h1>
            <p className="text-muted-foreground">Ready to climb the ranks today?</p>
          </div>
          <Link href="/lobby">
            <Button size="lg" className="h-16 px-8 text-lg font-bold gap-3 shadow-[0_0_20px_rgba(0,212,255,0.3)] hover:shadow-[0_0_30px_rgba(0,212,255,0.5)] transition-all" data-testid="button-quick-play">
        ...
        <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <StatCard title="Active Players" value={stats?.activePlayers || 0} icon={<Users className="text-primary" />} data-testid="stat-active" />
          <StatCard title="Total Matches" value={stats?.totalMatches || 0} icon={<Swords className="text-primary" />} data-testid="stat-total" />
        </section>
